...
#%% 
# '''Filtering by mean'''
# df, hr2,lr2=df.copy(deep=True), hr.copy(deep=True), lr.copy(deep=True)
...
for i in range(0,len(df_lr2.columns)):
    ...
    path_result ="C:/Users/maria/Documents/Marta Via/1. PhD/A. BCN_Series/MTR-PMF/Treatment/hi"
...
axis.text(x=0, y=35, s='Range:'+ ' [-'+str(num_r)+','+str(num_r)+']') # [-'+str(num_r)+','+str(num_r)+']',fontsize=10)
...
for i in range(0,len(df_hr_mean.columns)):
    ...
    values_a, bins_a, patches_a = plt.hist(HR_f, bins=int(0.1*len(df_hr_mean)))
    values_b, bins_b, patches_b = plt.hist(LR_f, bins=int(0.1*83))
    values_a2, values_b2=values_a/values_a.sum(), values_b/values_b.sum()
    hist_int=(histogram_intersection(values_a2, values_b2, 100,50))
    hist_intersection_r1.append(hist_int)
    hr_vals_a=pd.Series(values_a2, index=bins_a[:-1])
    ...
    if i==0:    
        fig2.savefig(path_result+'30m_'+str(i)+'.png')
#%%
...
axs.set_ylabel('Histogram Intersection (adim.)', fontsize=12)
axs2.set_ylabel('Scaled residuals \n standard deviation (adim.)', fontsize=12)
#%%
...
hm=axs.pcolor(hi, cmap='Greys',edgecolors='w', linewidth=2)
cb=plt.colorbar(hm)
cb.set_label('Histogram intersection (%)', fontsize=11)
axs.set_xticks(np.arange(0,13)+0.5)
axs.set_xticklabels(hi.columns,rotation=90)
axs.set_yticks(np.arange(0,7)+0.5)
axs.set_yticklabels(hi.index)
axs.set_xlabel('C values combinations',fontsize=12)
axs.set_ylabel('30 m Resolution',fontsize=12)
